...
class Encoder(nn.Module):
    def __init__(self, in_feat, hidden, z_dim):
        super().__init__()
        self.past_rnn = nn.GRU(input_size=in_feat, hidden_size=hidden, batch_first=True)
        self.fut_rnn = nn.GRU(input_size=1, hidden_size=hidden, batch_first=True)
...
class Decoder(nn.Module):
    def __init__(self, in_feat, hidden, z_dim, horizon):
        ...
        self.past_rnn = nn.GRU(input_size=in_feat, hidden_size=hidden, batch_first=True)
        self.fc_init = nn.Sequential(nn.Linear(hidden + z_dim, hidden), nn.ReLU())
        self.dec_rnn = nn.GRU(input_size=1, hidden_size=hidden, batch_first=True)
    ...
    def forward(self, past, z, teacher_future=None):
        ...
        x_t = torch.zeros(B, 1, 1, device=past.device)
        outs = []
        for t in range(self.horizon):
            y, h = self.dec_rnn(x_t, h)
            pred = self.out(y)
            outs.append(pred)
            x_t = teacher_future[:, t:t + 1, :] if teacher_future is not None else pred.detach()
...
def train_cvae(past_train, future_train, past_val, future_val, horizon,
               hidden=64, z_dim=16, epochs=10, batch=128, lr=1e-3, beta=0.1, seed=7):
    torch.manual_seed(seed)
    np.random.seed(seed)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = CVAE(in_feat=past_train.shape[-1], hidden=hidden, z_dim=z_dim, horizon=horizon).to(device)
...
def sample_future_returns(model, past_window_batch, z_dim, n_samples=200):
    ...
    with torch.no_grad():
        for _ in range(n_samples):
            ...
            gen = model.dec(past_window_batch, z, teacher_future=None)
